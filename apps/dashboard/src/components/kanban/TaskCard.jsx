...
export default function TaskCard({ task, pos, colId, onEdit, onMove, onDuplicate, isDoneCol }) {
    ...
    // left-border: green for Done column, deadline colour otherwise, neutral fallback
    ...
    // Collapsed indicator chips (shown when not hovered)
    ...
    return (
        <div
            ...
            onClick={onEdit}
            onMouseEnter={(e) => { setHovered(true); if (!dragging) { e.currentTarget.style.boxShadow = "0 8px 24px rgba(149,157,165,.15)"; e.currentTarget.style.transform = "translateY(-1px)"; } }}
        ...
        >
            {/* ── Main row ── */}
            <div style={{ padding: hovered ? "13px 14px 11px" : "9px 14px 9px", transition: "padding .2s" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    {/* drag handle */}
                    <div style={{ color: "#D1D5DB", fontSize: 15, lineHeight: 1, cursor: "grab", flexShrink: 0, opacity: hovered ? 1 : 0.4, transition: "opacity .2s" }}>⠿</div>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        {/* Collapsed micro-indicators */}
                        {!hovered && hasFoot && (
                            <div style={{ display: "flex", alignItems: "center", gap: 5, marginTop: 3, flexWrap: "nowrap", overflow: "hidden" }}>
                                {task.priority && (
                                    <span style={{
                                        fontSize: 9.5, fontWeight: 700, padding: "1px 6px", borderRadius: 4,
                                        textTransform: "uppercase", letterSpacing: ".05em",
                                        background: priColor.bg, color: priColor.fg,
                                    }}>{task.priority}</span>
                                )}
                                {task.tags?.length > 0 && (
                                ...
                                {db && (() => {
                                    ...
                                    return <span style={{ fontSize: 9.5, fontWeight: 600, color: st.fg }}>{db.text}</span>;
                                })()}
                            </div>
                        )}
                    </div>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 2, flexShrink: 0 }}>
                        {hovered && onDuplicate && (
                            <button
                                ...
                                style={{ width: 26, height: 26, border: "1px solid #E8EAED", background: "#F9FAFB", color: "#6B7280", cursor: "pointer", borderRadius: 7, display: "flex", alignItems: "center", justifyContent: "center", transition: "all .15s", padding: 0 }}
                ...
                {task.desc && (
                    <div style={{
                        ...
                        transition: "max-height .22s cubic-bezier(0.4,0,0.2,1), opacity .18s, margin-top .2s",
                    }}>
            ...
            {hasFoot && (
                <div style={{
                    ...
                    opacity: hovered ? 1 : 0,
                    overflow: "hidden",
                    transition: "max-height .22s cubic-bezier(0.4,0,0.2,1), opacity .18s",
                }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", padding: "8px 14px 12px", background: "#FAFBFD", borderTop: "1px solid #F0F1F3" }}>
                        ...
                        {task.priority && (
                            <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 9px", borderRadius: 5, textTransform: "uppercase", letterSpacing: ".06em", background: priColor.bg, color: priColor.fg }}>{task.priority}</span>
                        )}
                        {db && (() => {
                            const st = DL_BADGE_STYLE[db.type];
                            return <span style={{ fontSize: 10.5, padding: "2px 9px", borderRadius: 5, fontWeight: 600, background: st.bg, color: st.fg }}>{db.text}</span>;
                        })()}
                    </div>
                </div>
            )}
